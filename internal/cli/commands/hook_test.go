...
func TestHook_StatusBeforeInstall(t *testing.T) {
	...
	stdout, _, err := runVorCmd(t, nil, "hook", "status", tmp)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHook_InstallCreatesFile(t *testing.T) {
	tmp := gitRepoFixture(t)
	stdout, _, err := runVorCmd(t, nil, "hook", "install", tmp)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHook_InstallIsIdempotent(t *testing.T) {
	tmp := gitRepoFixture(t)
	for i := 0; i < 3; i++ {
		if _, _, err := runVorCmd(t, nil, "hook", "install", tmp); err != nil {
			t.Fatalf("install %d: %v", i, err)
		}
	}
	body, _ := os.ReadFile(filepath.Join(tmp, ".git", "hooks", "post-commit"))
	s := string(body)
	if got := strings.Count(s, "# vor-hook-start"); got != 1 {
		t.Errorf("# vor-hook-start count = %d after 3 installs, want 1", got)
	}
	if got := strings.Count(s, "# vor-hook-end"); got != 1 {
		t.Errorf("# vor-hook-end count = %d after 3 installs, want 1", got)
	}
}
...
func TestHook_InstallPreservesExistingHookContent(t *testing.T) {
	...
	if !strings.Contains(s, "# vor-hook-start") {
		t.Errorf("vor block not appended")
	}
}
...
func TestHook_StatusAfterInstall(t *testing.T) {
	...
	if !strings.Contains(stdout, "vor hook installed at") {
...
func TestHook_UninstallRemovesBlock(t *testing.T) {
	...
	stdout, _, err := runVorCmd(t, nil, "hook", "uninstall", tmp)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHook_UninstallPreservesOtherContent(t *testing.T) {
	...
	if _, _, err := runVorCmd(t, nil, "hook", "uninstall", tmp); err != nil {
		t.Fatal(err)
	}
	body, _ := os.ReadFile(hookPath)
	s := string(body)
	...
	if strings.Contains(s, "vor-hook-start") {
		t.Error("vor markers survived uninstall")
	}
}
...
func TestHook_FailsWithoutGitDir(t *testing.T) {
	...
	_, _, err := runVorCmd(t, nil, "hook", "install", tmp)
	if err == nil {
