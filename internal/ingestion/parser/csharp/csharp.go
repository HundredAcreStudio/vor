// Package csharp is the C# parser. Binds tree-sitter-c-sharp and handles
// classes, interfaces, structs, enums, records, methods, constructors,
// plus block + file-scoped (C# 10+) namespaces.
package csharp
...
func loadQuery() (*sitter.Query, error) {
	return common.LoadQueryOnce("csharp.scm", csharp.GetLanguage(),
		&compiledQueryOnce, &compiledQuery, &compiledQueryErr)
}

// Parser is the registered C# parser.
type Parser struct{}

func init() { parser.Register(langTag, &Parser{}) }

// Parse extracts C# symbols, using directives, and invocations. The extraction
// shell lives in common.RunQuery; this wires in C#'s symbol/import/call logic
// and the public-top-level export rule.
...
func absorbSymbol(out map[uint32]*models.Symbol, caps map[string][]*sitter.Node, fi models.FileInfo, source []byte) {
	...
	case "method_declaration", "constructor_declaration":
		complexity = 1 + common.CountBranchNodes(def, csharpBranchNodeTypes)
		nesting = common.MaxNestingDepth(def, csharpBranchNodeTypes)
	}
	...
	sym := &models.Symbol{
		...
		Visibility:         models.VisibilityInternal, // C# default for top-level types is internal
